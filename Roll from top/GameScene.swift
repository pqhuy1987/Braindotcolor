...
class GameScene: SKScene,SKPhysicsContactDelegate {
    ...
    var left_touchPoint = CGPoint.zero;
    var right_touchPoint = CGPoint.zero;
    ...
    override func didMove(to view: SKView) {
        ...
        left_quad1.physicsBody!.isDynamic = true
        ...
        right_quad1.physicsBody!.isDynamic = true
        ...
        if let games = defaults.string(forKey: "noofgames")
        {
            if ( Int(games) >= 3 )
            {
                tutorial_show = false;
        ...
        hint_label.fontColor = UIColor.gray
        ...
        left_quad1.run(SKAction.rotate(byAngle: CGFloat(-3 * M_PI_4), duration: 0))
        right_quad1.run(SKAction.rotate(byAngle: CGFloat(-M_PI_4), duration: 0))
    ...
    func spawnEnimes()
    {
        if(Double(framesCount) * gameLevel >=  ( 240 / gameLevel ) )
        {
           ...
           clone_obj.physicsBody!.isDynamic = true
           ...
           if( score > 10 )
           {
                ...
                toggler = !toggler;
                obstacleArray.append(obstacleStruct(left:toggler,sn: clone_obj,time: Int64(Date().timeIntervalSince1970*1000)));
           }
           else{
                obstacleArray.append(obstacleStruct(left:( arc4random_uniform(2) + 1 ) ==  1 ? true : false,sn: clone_obj,time: Int64(Date().timeIntervalSince1970*1000)));
            }
        ...
        if let touch = touches.first {
            ...
            if(touchLocation.x < size.width/2)
            {
                ...
                if(left_touchPoint.y < touchLocation.y)
                {
                    ...
                    left_quad1.run(SKAction.rotate(byAngle: CGFloat(M_PI_4/6), duration: 0.0))
                }
                else {
                    //left down
                    //print("Left Down");
                ...
                if(right_touchPoint.y < touchLocation.y)
                {
                    //print("Right Up");
                    //right up
                    right_quad1.run(SKAction.rotate(byAngle: CGFloat(-M_PI_4/6), duration: 0.0))
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first {
            ...
            if(touchLocation.x < size.width/2)
            {
                ...
                //print("Left Ended");
                left_touch = false;
                left_touchPoint = CGPoint.zero;
            }
            else if(touchLocation.x > size.width/2) {
                //Rigth
                //print("Right Ended");
                right_touch = false;
                right_touchPoint = CGPoint.zero;
            }
        }
    }
              ...
              (secondBody.categoryBitMask & right_bitmask != 0)) )
        {
            ...
            for obj in  obstacleArray {
                if( obj.spriteNode == firstBody.node)
                {
                    ...
                    if(( color == 1 && colorStr!.contains("red")) || (color == 2 && colorStr!.contains("blue")) || (color == 3 && colorStr!.contains("yellow")) || (color == 4 && colorStr!.contains("green"))) {
                        score += 1;
                        run(scored)
                    }
                    else
                    {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: time) {
                            self.defaults.setValue(String(Date().timeIntervalSince1970*1000), forKey: "updatedtime")
